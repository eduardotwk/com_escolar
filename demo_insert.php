...
$pais_establecimiento = pais_establecimiento($_SESSION["user"]);
...
$queryAnio = $con->query("SELECT ce_anio_curso FROM ce_curso WHERE id_ce_curso = $id_curso");
$row = $queryAnio->fetch(PDO::FETCH_ASSOC);
$anio = $row["ce_anio_curso"];
...
if ($cantidad != 0) {
    ...
    $resultado = valida_carga_excel($id_establecimiento, $id_docente, $id_curso, $anio);
